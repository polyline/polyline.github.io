...
目前更新到`v 0.0.5`版的各種感想，第一次認真的做一個網站，過程中真的有很多繁枝末節會讓人心很累

包括網頁的配色、排版，還有`class`的標籤要怎麼設比較好，像是設定顏色，我應該要把每種顏色獨自設成一個`class標籤`，還是分別在每個需要設定的`class`裡面設定`background-color`呢?

再來是`RWD`的設計也相當惱人，一開始只會朝向電腦版的設計，但往往到手機上是另外一回事，過程中就是要不斷的調整，非常消耗時間

等到排版及設計大致解決後，要怎麼為網站加一些小動畫是非常關鍵的
